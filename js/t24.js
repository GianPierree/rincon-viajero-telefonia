BX24.init(function () {
  ...
  $.ajax(campanaListar).done(function (response) {
    ...
    for (var key in objCampana) {
      ...
      htmlTbodyCampana += "<tr><th scope='row'>" + objCampana[key]["ID"] + "</th><td>" + objCampana[key]["NAME"] + "</td>---</td><td><button class='btn btn-info' role='button' onclick='campanaPlay(" + objCampana[key]["ID"] + ", 1)'><img src='./images/play.svg' /></button>  <button class='btn btn-info' role='button' onclick='campanaStop(" + objCampana[key]["ID"] + ", 0)'><img src='./images/stop.svg' /></button> <button class='btn btn-info' role='button' data-bs-toggle='modal' data-bs-target='#exampleModal' data-bs-whatever='" + objCampana[key]["ID"] +"'><img src='./images/edit.svg' /></button> <button class='btn btn-info' role='button' onclick='campanaTrash(" + objCampana[key]["ID"] + ")'><img src='./images/trash.svg' /></button></td></tr>";
